									...
									<c:forEach var="board" items="${boardList}">
										<tr>
											...
											<td class="date">
											 ...
											 <c:set var="news_imgctt" value="${board.news_imgctt.split(\";\")}"/>
											<td class="sbj">
												<div>
													<a href="${cp}/news/News_page.mo?news_idx=${board.news_idx}&page=${page}">
														<div class="tit">${board.news_title}</div>
														<p>${board.news_subtit}
														<c:forEach var="i" begin="0" end="${fn:length(news_imgctt)}" step="1">
														${news_imgctt[i]}
														</c:forEach>
														</p>
													</a>
											...
											<td class="more">
											<a href="${cp}/news/News_page.mo?news_idx=${board.news_idx}&page=${page}" class="btn">
												 <span>자세히보기</span>
												<div>
												</div>
											</a>
